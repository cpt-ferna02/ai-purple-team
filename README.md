...
### 85/100 Detection Coverage Score — T1003.001 LSASS Memory
![Coverage Score](screenshots/02-coverage-score.png)

### Simulation Commands + Detection Rules (Sigma / Splunk SPL / KQL / Wazuh)
![Simulation Commands and Detection Rules](screenshots/03-simulation-commands-detection-rules.png)

### Expected Log Evidence + Threat Hunting Queries
![Log Evidence and Hunt Queries](screenshots/04-log-evidence-hunt-queries.png)

### IOCs Generated + MITRE Mitigations
![IOCs and Mitigations](screenshots/05-iocs-mitigations.png)

### Coverage Analysis
![Coverage Analysis](screenshots/06-coverage-analysis.png)

---

## 📄 Sample PDF Report

This is a real exported report generated by the platform for technique **T1003.001 — LSASS Memory**.

It includes the executive summary, simulation commands, log evidence table, all four detection rules, detection gaps, hunt queries, and MITRE mitigations — all in a professionally formatted confidential document.

> 📥 **[Download Sample Report — purple_report_T1003.001.pdf](exports/purple_report_T1003.001.pdf)**
...
This was the biggest technical challenge of the project and required four separate debugging cycles to fully resolve.
...
```
Claude was returning a response where a special character inside a string value was terminating the JSON early. I added a fallback parser that tried to extract just the JSON object by finding the outermost `{` and `}` characters.

**Iteration 2 — Raw newlines inside strings:**
The error moved to a different position but same type — Claude was putting literal newline characters inside JSON string values (in multi-line Sigma rules), which is invalid JSON. I added a regex to strip them:
```python
raw = re.sub(r'(?<!\\)\n(?!["\s*\}\]\{])', ' ', raw)
```
This helped but didn't catch all cases.

**Iteration 3 — Unescaped quotes breaking the delimiter:**
```
json.JSONDecodeError: Expecting ',' delimiter: line 32 column 6
```
Detection rules (Sigma YAML, KQL, Wazuh XML) contain double quotes and backslashes in their syntax. Claude was including these literally inside JSON strings instead of escaping them. I wrote a character-level state machine that walked through the raw response tracking whether each character was inside a JSON string, replacing any raw newlines it found:
...
Rather than only fixing the parser, I also updated the prompt itself to instruct Claude to return all detection rules as single-line escaped strings. Fixing both the output format and the parser together made the solution robust.

**What I learned:** When an LLM returns structured data, you cannot assume the output will be perfectly formatted even when explicitly asked for JSON. The right approach is to sanitize the response defensively before parsing, and also engineer the prompt to reduce the problem at the source. A state machine is more reliable than regex for parsing nested structured text.
...
**How I solved it:** Wiped the entire git history by deleting the `.git` folder and reinitializing from scratch. I also immediately rotated the exposed API key.
...
**What I learned:** Secret exposure is permanent in git history unless you rewrite it. The `.gitignore` must exist and be verified before any `git add .` is ever run. Rotating the compromised key is mandatory — deleting the file is not enough.
...
**Root cause:** The `.gitignore` file had been created as a 0-byte empty file and never had content written to it properly.
...
**What I learned:** Always run `git status` and visually verify the file list before every `git add .`. A `.gitignore` that looks correct in the file explorer may have been saved incorrectly.

---

### 🟡 Challenge 5 — PowerShell vs CMD Syntax

**What happened:** Following instructions to delete the `.git` folder:

```
Remove-Item: A positional parameter cannot be found that accepts argument '/q'.
```

**Root cause:** VS Code was running PowerShell, not Command Prompt. The `rmdir /s /q` syntax is CMD-only.
...
**What I learned:** Windows has two separate terminal environments with different syntax. When a command fails unexpectedly in PowerShell, check whether it's a CMD-style command that needs a PowerShell equivalent.
